...
class MyApplication(QMainWindow):
    ...
    def validation_form(self, list_data, type_form):
        if type_form == 2 or type_form == "2":
            ...
            if len(value_gluebox) <= 6:
                list_error_msg.append("Validation Error (length of string <=6 ?)")
    ...
    def generate_zpl_2(self, list_data: list):
        ...
        # print(arr_printed)
        arr_printed = [] # test
        index = len(arr_printed)
        if len(arr_printed) >= len(stepcodes):
            print("ERROR: Không còn stepcode chưa in"
            )
            createLog(f"KHÔNG CÒN STEPCODE CHƯA IN VỚI: {value_GlueBox}", "DT","E")
            return [None, None]
        ...
        l.custom_barcode_field_default(self.addby_barcode_2, height=self.height_barcode_2)
        ...
        l.origin(self.origin_X_2 + self.ml_desc_2, self.origin_Y_2+ self.mt_desc_2 + self.line_space_2)
        l.write_text(
            text=data_print,
    ...
    def handle_notification(self, data: tuple):
        ...
        form_name = self.uic.cb_TypeStamp.currentText()
        if form_name == "GLUE BOX":
            self.uic.txt_Box_DataPrint.setPlainText(data)
